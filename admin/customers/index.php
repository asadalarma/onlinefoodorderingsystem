                            ...
                            <div class="card-content collapse show">
                                <div class="card-body card-dashboard">
                                    ...
                                    <table class="table table-striped table-bordered file-export">
                                        ...
                                        <tbody>
                                        <?php
                                        $customersquery = "select * from users where user_type='customer' and is_deleted=0";
                                        $customersquery_result = mysqli_query($conn, $customersquery);
                                        $count = mysqli_num_rows($customersquery_result);
                                        if ($count) {
                                            while ($row = mysqli_fetch_assoc($customersquery_result)) {
                                                echo '<tr>
                                            <td>' . $row["name"] . '</td>
                                            <td>' . $row["email"] . '</td>
                                            <td>' . $row["address"] . '</td>
